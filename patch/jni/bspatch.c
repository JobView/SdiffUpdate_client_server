...
int bspatch_main(int argc,char * argv[])
{
	...
	if(argc!=4) printf("usage: %s oldfile newfile patchfile\n",argv[0]);
	...
	*/
	...
	if(((fd=open(argv[1],O_RDONLY,0))<0) ||
	   ...
	   (close(fd)==-1)) printf("%s",argv[1]);
	if((new=malloc(newsize+1))==NULL) printf(NULL);
...
//合并
JNIEXPORT void JNICALL Java_com_wzf_patch_PatchHelper_patch
